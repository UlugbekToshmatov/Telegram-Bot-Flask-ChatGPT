...
# Dictionary to store temp_message for each chat
temp_messages = {}
user_role = None
edit_message = True
# temp_message = None
...
async def user_prompt_handler(message: types.Message, session: AsyncSession, bot: Bot, state: FSMContext):
    ...
    if question_state == QuestionFSM.in_progress:
        await message.answer(
            text="Iltimos, boshqa savollaringgizni javobimdan so'ng yozing",
    ...
    print(f'user of message: {message.from_user}')
    ...
    try:
        ...
        user_message = await save_message(
            session=session,
            message=Message(text=text, sender=SenderType.USER.name, tg_message_id=message.message_id, chat_id=chat.id)
        )
    except Exception as e:
        print(e)
    ...
    # Send "Iltimos, kutib turing" messages in a separate thread
    async def send_waiting_messages():
        try:
            temp_message = await message.answer(
                text='Iltimos, kutib turing.',
                reply_to_message_id=message.message_id
            )
            temp_messages[chat_id] = temp_message.message_id
            await asyncio.sleep(1)

            if edit_message:
                await bot.edit_message_text(
                    text='Iltimos, kutib turing..', chat_id=chat_id, message_id=temp_messages[chat_id]
                )
                await asyncio.sleep(1)
            ...
            print(f"Error sending wait messages: {exc}")
            await message.answer(
                text="Kechirasiz, texnik nosozlik yuz berdi! Iltimos, yana bir urinib ko'ring.",
                reply_to_message_id=message.message_id
            )
            return

    # Create and run both threads concurrently
    async def process_openai_response():
        ...
        if chat.asst_run_id is None:
            response = await send_async_message_to_open_ai(text=masked_question, thread_id=chat.asst_thread_id)
        else:
            response = await send_async_message_to_open_ai(text=masked_question, thread_id=chat.asst_thread_id,
                                                           run_id=chat.asst_run_id)
        ...
        if assistant_run_id is not None:
            print(f"Saving last run_id={assistant_run_id} for chat={chat.id}")
            await update_chat_run_id(session=session, chat_id=chat.id, data={'asst_run_id': assistant_run_id})
        else:
            # If no run_id is returned, then an error occurred in OpenAI while cancelling run. Thus, create
            # a new thread to send messages to OpenAI through.
            new_asst_thread_id = await create_thread()
            chat.asst_thread_id = new_asst_thread_id
            await update_chat(session=session, chat_id=chat.id, data={'asst_thread_id': new_asst_thread_id})

            edit_message = False
            await bot.edit_message_text(text=assistant_response, chat_id=chat_id, message_id=temp_messages[chat_id])
            return
        ...
        tg_bot_response = await bot.edit_message_text(
            text=assistant_response, chat_id=chat_id, message_id=temp_messages[chat_id],
            reply_markup=get_callback_buttons(buttons={
        ...
        await save_tg_message_id(session=session, message_id=assistant_message.id,
                                 tg_message_id=tg_bot_response.message_id)
        question_state_ = await state.get_state()
        if question_state_ == QuestionFSM.in_progress:
            await state.clear()
        print(f"Time spent in total: {(end - start)} seconds")

    # Run both tasks concurrently
    await asyncio.gather(send_waiting_messages(), process_openai_response())
...
#
#         # Store the temp_message_id for the specific chat_id
#         temp_messages[chat_id] = temp_message.message_id
#         print(f"Temp messages: {temp_messages}")
...
#         response = await send_async_message_to_open_ai(text=masked_question, thread_id=chat.asst_thread_id)
#     else:
#         response = await send_async_message_to_open_ai(text=masked_question, thread_id=chat.asst_thread_id, run_id=chat.asst_run_id)
#     assistant_response = response['assistant_response']
#     assistant_message_id = response['assistant_message_id']
#     assistant_run_id = response['assistant_run_id']
#
#     try:
#         await bot.edit_message_text(text='Iltimos, kutib turing..', chat_id=chat_id, message_id=temp_messages[chat_id])
#     except TelegramBadRequest as e:
#         print(f"Error editing temp message: {e}")
#         await message.answer("Xatolik yuz berdi, men xabarni tahrirlashni amalga oshira olmadim.")
#         return
#
#     # Save last run id to retrieve and cancel a potential 'in_progress' run if no error occurred in OpenAI
#     if assistant_run_id is not None:
...
#         await bot.edit_message_text(text='Iltimos, kutib turing...', chat_id=chat_id, message_id=temp_messages[chat_id])
#     except TelegramBadRequest as e:
#         print(f"Error editing temp message 2: {e}")
#         await message.answer("Xatolik yuz berdi, men xabarni tahrirlashni amalga oshira olmadim.")
#         return
#
#     if is_cyrillic_text:
